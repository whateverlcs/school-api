...
{
    public class GetAcademyByCityUseCase : IGetAcademyByCityUseCase
    {
        private readonly IMapper _mapper;
        private readonly IAcademyReadOnlyRepository _repository;

        public GetAcademyByCityUseCase(IMapper mapper, IAcademyReadOnlyRepository repository)
        {
            _mapper = mapper;
            _repository = repository;
        }

        public async Task<IList<ResponseAcademyJson>> Execute(string cityName)
        {
            var academy = await _repository.GetByCity(cityName);
